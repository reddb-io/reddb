...
fn main() {
    ...
    for sql in [
        ...
        "SELECT url, status, body_bytes, inline_body_bytes, truncated FROM crawlex_raw_response_rows LIMIT 10",
        "SELECT COUNT(*) AS n FROM crawlex_rendered_page_rows",
        "SELECT url, status, bytes, html_bytes, inline_html_bytes, truncated FROM crawlex_rendered_page_rows LIMIT 10",
