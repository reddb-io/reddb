...
- **Metric descriptor** — WAL-backed analytics catalog record keyed by a stable dotted metric path (for example `infra.database.cpu.usage`) and carrying the metric type/kind (`counter`, `gauge`, `histogram`), unit, dimensions, retention, rollup, and cardinality policy. `CREATE METRIC <path> TYPE <kind>` may exist as ergonomic sugar, but the canonical state is an ordinary selectable/updatable catalog record governed by RedDB policies.
