...
The first queue-wait slice should keep that boundary even when it spans HTTP, RedWire, gRPC, and Postgres-wire: Postgres-wire may carry RedDB `QUEUE READ ... WAIT` cancellation/return behavior, but PG-like `LISTEN`/`NOTIFY` belongs to the later ephemeral-notification slice.
