...
- **Signal plane** — SWIM-style peer-to-peer dissemination layer among admitted cluster members. It carries only liveness observations, member health inputs, load metrics, and catalog-version/topology hints; membership admission, ownership transitions, votes, and bootstrap state stay on their authoritative control-plane paths.
...
- **Cooperative lease handoff** — planned availability path where the current writer, the target, and the control plane coordinate before the current lease expires so a healthy successor can take over with minimal pause. The handoff still completes through the authoritative ownership/promotion transition; peer hints may only accelerate readiness checks and routing refresh.
- **Failover profile** — named timing and safety-policy bundle for liveness suspicion, lease duration, promotion grace, retry, and freshness windows. Profiles tune availability behavior for deployment posture without weakening commit policy, ownership fencing, or promotion-safety requirements.
- **Spread rule** — placement preference or constraint that distributes owners and replicas across members, zones, racks, or other failure domains. A spread rule guides placement and rebalancing plans; it is not itself ownership authority.
