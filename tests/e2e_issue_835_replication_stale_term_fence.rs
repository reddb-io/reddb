...
use reddb::replication::fence::{FenceBoundary, StreamHandshake, TermFence};
...
fn stale_term_handshake_is_refused_at_the_handshake_boundary() {
    let new_term = induce_failover_to_new_term(5, 42);
    let fence = TermFence::new(new_term);
    ...
    let rejection = fence
        ...
        .expect_err("stale handshake must be refused");
...
fn stale_term_lease_holder_fails_closed() {
    ...
    let promoted = store
        .try_acquire_for_term("main", "new-primary", new_term, 1)
    ...
    let err = store
        .try_acquire_for_term("main", "old-primary", 5, 60_000)
    ...
    let guard = stale_lease.guard_current_term(new_term);
    assert!(
        matches!(guard, Err(LeaseError::FencedByTerm { .. })),
        "a lease on the old term must fail closed at mutate time: {guard:?}"
    );
    ...
    let resynced = store
        .try_acquire_for_term("main", "old-primary", new_term, 1)
...
fn returning_ex_primary_is_fenced_everywhere_then_admitted_after_resync() {
    ...
    let _new_primary_lease = store
        .try_acquire_for_term("main", "new-primary", new_term, 60_000)
        .unwrap();

    // --- stale ex-primary fenced at every boundary ---
    assert_eq!(
        fence
            .admit_handshake(&StreamHandshake::new("old", stale_term))
            .unwrap_err()
            .boundary,
    ...
    assert!(
        store
            .try_acquire_for_term("main", "old", stale_term, 60_000)
            .is_err(),
    ...
    let mut adopting = TermFence::new(stale_term);
    assert!(adopting.adopt_term(new_term), "adopts the newer term");
    assert!(fence
        .admit_handshake(&StreamHandshake::new("old", new_term))
        .is_ok());
    applier
