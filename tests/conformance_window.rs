//! Issue #592 — Analytics slice 7d conformance harness.
//!
//! Each pair under `tests/conformance/window/sql/<name>.sql`
//! plus `tests/conformance/window/expected/<name>.out` is run
//! against an in-memory RedDB runtime; the rendered output is
//! diffed against the frozen golden hand-computed from the
//! PG 17 manual (§4.2.8 + §3.5) per the HITL decision recorded
//! on the issue.
...
fn window_conformance_corpus_matches_pg17() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/conformance/window");
    let cases = discover_cases(&root.join("sql"), &root.join("expected"))
        .expect("discover conformance cases");
    assert!(
        cases.len() >= 10,
        "conformance corpus must have at least 10 cases (found {})",
        cases.len()
    );
    ...
    let rows = result
        ...
        .map(|record| {
            columns
                .iter()
                .map(|column| {
                    format_conformance_value(record.get(column.as_str()).unwrap_or(&Value::Null))
                })
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
...
fn extract_message(records: &[UnifiedRecord]) -> Option<String> {
    ...
    record.get("message").map(format_conformance_value)
}

fn format_conformance_value(value: &Value) -> String {
    match value {
