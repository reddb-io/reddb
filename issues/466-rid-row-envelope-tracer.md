...
- [ ] Row query results expose `rid`, `collection`, `kind`, `tenant`, `created_at`, and `updated_at`.
- [ ] `SELECT *` and `RETURNING *` on rows include the public item envelope.
- [ ] Row `kind` is `row`.
- [ ] Public row query JSON uses `rid` as the RedDB ID field.
- [ ] Existing row update/select tests are updated to use `rid` where they target the public identifier.
- [ ] Docs touched by the row tracer use RedDB ID / `rid` vocabulary.
