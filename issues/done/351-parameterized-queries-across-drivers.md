...
**ADR.** ADR 0015 (`docs/adr/0015-parameterized-query-contract.md`) codifies: placeholder syntax, `Value` enum surface, wire frame layout, capability negotiation, error taxonomy, and the deprecation policy for unsafe string-concat patterns in docs and examples. It was accepted through the human gates in #352 and #377.
...
- ADR 0015 is the accepted recorded contract for this PRD and closes the human gates #352 and #377.
- A child PRD handles documentation sweep across `docs/` so the parameterized form becomes the default in every example.

## Completion

Accepted during human review on 2026-05-14 after #352 and #377 were accepted.
