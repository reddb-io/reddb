...
- [ ] `cargo check` passes. — Not executed (sandbox blocks `cargo`). Run `cargo check -p reddb-server` and `cargo test -p reddb-server --lib evaluator` to verify.
...
- Compiled-filter bypass precondition now documented in `filter_compiled.rs` module doc. The precondition is: `Filter` predicates carry pre-typed values, so no implicit-cast step is needed; all `FilterOp` variants are enumerated; `compiled_matches_legacy_for_random_filters` guards parity.
- Next slice: replace the inline projection arms in `query/core.rs` / `query/executor.rs` with `evaluator::evaluate` calls; wire the WHERE-clause SQL path through the evaluator (`filter.rs::Predicate` is a lower-level API that predates the SQL path — migrate the SQL lowering in `sql_lowering.rs` to produce `Expr` trees and evaluate via `evaluator::evaluate`).
