...
- [x] `AuditRecordBuilder` deep module: unit tests verifying every field, `answer_hash` is deterministic SHA-256, `include_answer` toggles content vs hash.
- [x] `red_ask_audit` collection auto-created on first ASK.
- [x] Audit row written before answer returns to client.
- [ ] Retention purge runs per `ask.audit.retention_days`.
- [x] Integration test: 5 ASK calls produce 5 audit rows with correct fields.
- [x] Integration test: `include_answer` flag toggle changes stored shape.
...
Slice 2: ASK runtime audit wiring landed.

- `execute_ask` now builds an `AuditRecordBuilder::CallState` after
  citation parsing and writes one row to `red_ask_audit` before
  constructing the client-visible `RuntimeQueryResult`.
- `red_ask_audit` is auto-created on first ASK via
  `get_or_create_collection`, then inserted through
  `create_rows_batch` with events suppressed so the write still uses
  the normal mutation/index/CDC path without event fanout.
- `ask.audit.include_answer` is read from `red_config` through
  `config_bool`; default `false` stores only `answer_hash`, while
  `true` adds the full `answer` field.
- ASK statements are treated as result-cache-volatile so repeated
  identical ASK calls cannot bypass `execute_ask` and skip audit.
- Integration coverage in `tests/ask_audit.rs` now pins:
  - first ASK creates/queryables `red_ask_audit` and writes required
    fields before return;
  - `include_answer` changes the stored shape;
  - five identical ASK calls append five audit rows.

Verification for slice 2:
- `cargo test --test ask_audit`
- `cargo check`
- `git diff --check`
- `pnpm test` (skipped: target/debug/red missing)
- `pnpm typecheck` (nonzero wrapper exit after reporting
  `TypeScript: No errors found`)

Known follow-up:
- `ask.audit.retention_days` purge remains unwired.
- Cost accounting still records `cost_usd = 0.0` until the cost-guard
  slice is integrated with `execute_ask`.
- Effective strict/lenient mode and retry count remain hard-coded to
  the current non-retry path until #395/#396 wiring lands.
- `cargo test -p reddb-io-server
  ask_statements_do_not_read_or_write_result_cache` was blocked before
  reaching the new test by pre-existing E0716 borrow errors in
  `runtime/ai/pg_wire_ask_row_encoder.rs` unit tests.
