...
mod watch_path_tests {
    use super::kv_watch_path;

    #[test]
    fn parses_single_key_watch_path() {
        assert_eq!(
            kv_watch_path("/collections/settings/kv/theme/watch"),
            Some(("settings", "theme"))
        );
        assert_eq!(kv_watch_path("/collections/settings/kv/theme"), None);
        assert_eq!(kv_watch_path("/collections//kv/theme/watch"), None);
    }
}
