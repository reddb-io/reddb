...
impl<'a> Parser<'a> {
        ...
        match self.peek() {
            ...
            other => Err(ParseError::expected(
                vec![
                    ...
                    "DELETE", "PUT", "GET", "CAS", "INCR", "DECR", "CREATE", "DROP", "ALTER",
    ...
    pub fn parse_sql_command(&mut self) -> Result<SqlCommand, ParseError> {
        match self.peek() {
            ...
            Token::Ident(name) if name.eq_ignore_ascii_case("CAS") => {
                match self.parse_kv_cas_query()? {
                    QueryExpr::Kv(query) => Ok(SqlCommand::Kv(query)),
                    other => Err(ParseError::new(
                        format!("internal: CAS produced unexpected query kind {other:?}"),
                        self.position(),
                    )),
                }
            }
            Token::Ident(name) if name.eq_ignore_ascii_case("INCR") => {
                match self.parse_kv_incr_query()? {
                    QueryExpr::Kv(query) => Ok(SqlCommand::Kv(query)),
                    other => Err(ParseError::new(
                        format!("internal: INCR produced unexpected query kind {other:?}"),
                        self.position(),
                    )),
                }
            }
            Token::Ident(name) if name.eq_ignore_ascii_case("DECR") => {
                match self.parse_kv_decr_query()? {
                    QueryExpr::Kv(query) => Ok(SqlCommand::Kv(query)),
                    other => Err(ParseError::new(
