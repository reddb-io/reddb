...
impl IvfIndex {
    ...
    ///
    /// The on-disk byte layout lives in [`reddb_file::vector_ivf_index`]; this
    /// method only projects engine state into the codec frame.
    ...
    ///
    /// Byte parsing lives in [`reddb_file::vector_ivf_index`]; this method
    /// rebuilds engine state, including the derived id→list lookup.
