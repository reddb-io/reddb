...
use crate::application::collection_contract_enforcer::{
    normalize_row_update_assignment_with_plan, CollectionContractWriteEnforcer,
};
...
impl RedDBRuntime {
    ...
    pub(crate) fn apply_loaded_sql_update_row_core(
        ...
        // Documents store the full JSON under the `body` column. Legacy rows
        // also promote top-level keys for filtering, while binary body mode
        // keeps `body` as the single source of truth. Reflect non-body SET
        // assignments into `body` as patch operations so dotted paths update
        // nested JSON instead of becoming literal top-level keys.
        let is_document_collection = db
            .collection_contract(&collection)
...
impl RuntimeEntityPort for RedDBRuntime {
    ...
    fn create_rows_batch_prevalidated_with_outputs(
        ...
        let default_ttl_plan = contract.default_ttl_plan();
        ...
        for row in input.rows {
            ...
            let mut metadata = row.metadata;
            default_ttl_plan.apply(&mut metadata);
