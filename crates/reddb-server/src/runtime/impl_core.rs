...
impl RedDBRuntime {
    ...
    pub(crate) fn config_f32(&self, key: &str, default: f32) -> f32 {
        if let Some(raw) = self.inner.env_config_overrides.get(key) {
            return raw.parse::<f32>().unwrap_or(default);
        }
        let store = self.inner.db.store();
        let Some(manager) = store.get_collection("red_config") else {
            return default;
            ...
            if let Some(row) = entity.data.as_row() {
                ...
                if entry_key == Some(key) {
                    ...
                    if id >= latest_id {
                        latest_id = id;
                        result = match row.get_field("value") {
                            Some(crate::storage::schema::Value::Float(n)) if n.is_finite() => {
                                *n as f32
                            }
                            Some(crate::storage::schema::Value::Integer(n)) => *n as f32,
                            Some(crate::storage::schema::Value::UnsignedInteger(n)) => *n as f32,
                            Some(crate::storage::schema::Value::Text(s)) => {
