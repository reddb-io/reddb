...
fn compact_vcs_entity_json(entity: &UnifiedEntity) -> JsonValue {
    ...
    object.insert(
        ...
        JsonValue::String(vcs_entity_type(entity).to_string()),
    );
    object.insert(
        "red_capabilities".to_string(),
        JsonValue::Array(
            vcs_entity_capabilities(entity)
                .into_iter()
    ...
    match &entity.data {
        EntityData::Row(row) => {
            if let Some(named) = &row.named {
                object.insert(
                    "row".to_string(),
                    JsonValue::Object(vcs_named_fields_json(named)),
                );
            }
        }
        ...
        EntityData::Edge(edge) => {
            object.insert(
                "weight".to_string(),
                JsonValue::Number(f64::from(edge.weight)),
            );
            object.insert(
                "properties".to_string(),
                JsonValue::Object(
            ...
            object.insert(
                ...
                JsonValue::Array(
                    vector
                        .dense
                        .iter()
                        .map(|value| JsonValue::Number(f64::from(*value)))
                        .collect(),
                ),
            );
        ...
        EntityData::TimeSeries(timeseries) => {
            object.insert(
                "metric".to_string(),
                JsonValue::String(timeseries.metric.clone()),
            );
            object.insert(
                "timestamp_ns".to_string(),
                JsonValue::Number(timeseries.timestamp_ns as f64),
            );
            object.insert("value".to_string(), JsonValue::Number(timeseries.value));
        }
        EntityData::QueueMessage(message) => {
            object.insert("payload".to_string(), message.payload.to_json());
            object.insert(
                "attempts".to_string(),
                JsonValue::Number(f64::from(message.attempts)),
            );
            object.insert("acked".to_string(), JsonValue::Bool(message.acked));
        }
    }
...
fn vcs_entity_type(entity: &UnifiedEntity) -> &'static str {
    match (&entity.kind, &entity.data) {
        (EntityKind::TableRow { .. }, EntityData::Row(row)) if vcs_row_is_kv(row) => "kv",
...
fn vcs_entity_capabilities(entity: &UnifiedEntity) -> Vec<String> {
    let capabilities = match (&entity.kind, &entity.data) {
        (EntityKind::TableRow { .. }, EntityData::Row(row)) => {
            let mut values = BTreeSet::from(["table".to_string(), "structured".to_string()]);
            if vcs_row_is_kv(row) {
                values.insert("kv".to_string());
            }
            if row
                .named
                .as_ref()
                .is_some_and(|named| named.values().any(vcs_documentish_value))
                || row.columns.iter().any(vcs_documentish_value)
            {
...
fn vcs_documentish_value(value: &Value) -> bool {
    matches!(value, Value::Json(_) | Value::Blob(_))
}

fn vcs_row_is_kv(row: &RowData) -> bool {
    let Some(named) = row.named.as_ref() else {
        return false;
    };
    match named.len() {
        1 => named.contains_key("key") || named.contains_key("value"),
        2 => named.contains_key("key") && named.contains_key("value"),
        _ => false,
    }
}

fn vcs_named_fields_json(named: &HashMap<String, Value>) -> Map<String, JsonValue> {
    let mut object = named
        .iter()
        .map(|(key, value)| (key.clone(), value.to_json()))
        .collect::<Map<_, _>>();
    if let Some(Value::Json(bytes)) = named.get("body") {
        if let Some(fields) = crate::document_body::body_fields(bytes) {
            for (key, value) in fields {
                object.entry(key).or_insert_with(|| value.to_json());
            }
        }
    }
    object
...
fn materialize_merge_conflicts(
    ...
    for coll in store.list_collections() {
        ...
        for entity in manager.query_all(|_| true) {
            ...
            let body = compact_vcs_entity_json(&entity);
