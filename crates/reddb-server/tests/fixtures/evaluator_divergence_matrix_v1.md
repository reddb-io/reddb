...
The table covers the known LIKE case/byte, NULL equality and 3VL, overflow, division-by-zero, numeric coercion, decimal, Unicode, temporal, missing-column, and Boolean families. `unsupported` means that the implementation has no representation for that operation or value family.
...
| NULL / 3VL / missing column = NULL | `error:missing-column` | `none` | `none` | `false` | `false` | `true` | `unsupported` | `unsupported` | `unsupported` |
| NULL / 3VL / missing column IS NULL | `error:missing-column` | `true` | `compile-error:scalar compile: unsupported shape (IS NULL)` | `true` | `true` | `true` | `unsupported` | `unsupported` | `unsupported` |
