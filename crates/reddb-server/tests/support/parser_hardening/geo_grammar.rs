...
///
/// The integer portion is restricted to `[0, 89]` so the fractional
/// jitter (`0..1_000_000`) cannot push the literal out of range; the
/// edge values `±90.0` are exercised by hand-rolled happy-path tests
/// so the strategy does not need to emit them itself.
pub fn lat_lit() -> impl Strategy<Value = String> {
    (any::<bool>(), 0u32..=89, 0u32..1_000_000u32).prop_map(|(neg, deg, frac)| {
...
/// the western hemisphere is covered by the proptest sweep. The
/// integer portion is restricted to `[0, 179]` so the fractional
/// jitter cannot push the literal out of range.
pub fn lon_lit() -> impl Strategy<Value = String> {
    (any::<bool>(), 0u32..=179, 0u32..1_000_000u32).prop_map(|(neg, deg, frac)| {
        let sign = if neg { "-" } else { "" };
        format!("{}{}.{:06}", sign, deg, frac)
    })
}
