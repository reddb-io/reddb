...
/// Syntactic binary operators — the operator vocabulary.
///
/// `BinOp` was re-homed into the neutral keystone crate
/// [`reddb_types::operator`] (ADR 0052): the coercion spine that keys
/// overload resolution on it now lives there, and leaving the enum behind
/// would force that crate to depend back on `reddb-server`. This re-export
/// keeps the historical `ast::BinOp` path resolving so every parser /
/// analyzer / evaluator call-site stays untouched.
pub use reddb_types::operator::BinOp;
