...
| PSC-004 | docs/query/insert.md | INSERT can create rows, documents, and native timeseries points. | FB-OLD-19, FB-OLD-28, FB-NEW-10, FB-NEW-21 | passing | runtime/parser | Document INSERT/CRUD is covered by `tests/grouped/docs_kv_queue/e2e_documents_first_class_crud.rs`; cross-transport document CRUD parity is covered by `tests/grouped/surface_contracts/cross_transport_result_equivalence.rs`. |
...
| PSC-006 | docs/query/insert.md | Timeseries stores timestamped metrics with tags and supports useful query/readback. | FB-OLD-31, FB-OLD-32, FB-NEW-15, FB-NEW-16 | passing | runtime/parser | Timeseries readback is covered by `tests/grouped/timeseries_metrics/e2e_create_hypertable.rs`; automatic columnar projection, floor behavior, opt-out, and `COLUMNAR` rejection are covered by `crates/reddb-server/tests/columnar_activation_e2e.rs` and `crates/reddb-server/tests/timeseries_parser.rs`. |
...
| PSC-012 | ../feedbacks-new.md | Server transports expose the same query contract as embedded. | FB-NEW-03, FB-NEW-24, FB-NEW-25, FB-NEW-26, FB-NEW-27, FB-NEW-28, FB-NEW-29, FB-NEW-30 | passing | transport smoke | Document CRUD now has embedded, HTTP, gRPC, RedWire, and PG-wire equivalence coverage in `tests/grouped/surface_contracts/cross_transport_result_equivalence.rs`. |
...
| PSC-018 | ../feedbacks-new.md | Native statistics, vector, ASK, and SEARCH surfaces should either work or be documented as limited. | FB-OLD-18, FB-NEW-12, FB-NEW-17, FB-NEW-19, FB-NEW-23, FB-NEW-33 | failing | runtime/parser | Close statistics/vector/search issues or remove public promises from docs/examples until supported. |
...
| FB-OLD-18 | ../feedbacks.md | CREATE VECTOR was rejected while advertised by parser/docs. | PSC-018 | Needs vector slice or docs correction. |
| FB-OLD-19 | ../feedbacks.md | CREATE DOCUMENT was rejected. | PSC-004 | Cleared by document CRUD conformance, including partial-update sibling preservation and persistence across reopen. |
...
| FB-NEW-23 | ../feedbacks-new.md | VECTOR keyword existed but CREATE VECTOR was rejected. | PSC-018 | Needs vector support or docs removal. |
