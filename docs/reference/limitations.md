...
| Hypertables (time-range partitioning) | Shipped (minimal DDL) | `CREATE HYPERTABLE name TIME_COLUMN ts CHUNK_INTERVAL '1d' [TTL '7d']` registers a `HypertableSpec`; column-list form (`CREATE HYPERTABLE name (col TYPE, ...) CHUNK_INTERVAL ...`) is **planned**. See [hypertables.md](../data-models/hypertables.md). |
| Partition TTL | Shipped via `TTL` keyword | `CREATE HYPERTABLE ... TTL '90d'` attaches a default TTL to each new chunk. Per-chunk overrides + `chunks_expiring_within` preview live on the `HypertableRegistry` programmatic API. See [Partition TTL](../data-models/partition-ttl.md). |
...
- Distributed query execution + production automatic sharding runtime (see
