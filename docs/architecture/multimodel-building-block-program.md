...
| Query performance | Prior vector improvements retained; [runtime graph kind filtering](../perf/graph-materialization-filter.md) avoids unrelated payload copies and CALL candidate IDs, including in mixed collections | Stable snapshot cursors without O(N) ID arrays or scoring locks; projection/filter pushdown; common multimodel snapshot/auth and bounded memory |
