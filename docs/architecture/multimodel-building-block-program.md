...
| Query performance | Prior vector improvements retained; graph materialization filters entity kinds before payload copying and CALL candidate-ID collection, including in mixed collections | Stable snapshot cursors without O(N) ID arrays or scoring locks; projection/filter pushdown; common multimodel snapshot/auth and bounded memory |
