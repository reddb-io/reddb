...
and decimals use `$int`, `$uint` and `$decimal` envelopes. Restore binds these
values as query parameters, preserving quotes, backslashes and Unicode without
SQL-literal escaping. Earlier dumps containing SQL-formatted strings cannot be
...
This correction covers JSON row values, not complete database backup fidelity.
...
(for example UUID versus text, or timestamp versus integer). Non-row multimodel
