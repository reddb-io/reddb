...
- one encoded value for every RedWire `Value` / gRPC `QueryValue` tag: null,
  bool, int, float, text, bytes, vector, json, timestamp, and uuid
...
1. Add a new manifest entry with a stable `name`, `kind`, and lowercase
   `redwire_hex`, and `grpc_hex`.
2. Add at least one query payload fixture if the change affects the
   `QueryWithParams` frame layout or gRPC `QueryRequest` serialization.
...
against the Rust gRPC client, JS, Go RedWire/gRPC, Java, Kotlin, .NET, Dart,
PHP, Zig, and C++ codecs. Other driver tests should use the same manifest
rather than copying expected bytes inline.
