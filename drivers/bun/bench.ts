#!/usr/bin/env bun
/**
 * RedDB Bun Wire Protocol Benchmark
 * Usage: bun run bench.ts [host:port]
 */
...
const rqs = Array.from({ length: RANGE_QUERIES }, () => {
  const l = 18 + (rand() % 53); return [l, l + 10] as const
})
t0 = performance.now()
...
// Filtered queries
const fqs = Array.from({ length: FILTER_QUERIES }, () => [cities[rand() % 5], 18 + (rand() % 43)] as const)
t0 = performance.now()
