...
func TestParseURI_TableDriven(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "red proto=https default port",
			...
			w:   want{kind: KindHTTPS, host: "host", port: 55555}},
		{name: "red proto=http default port",
			uri: "red://host?proto=http",
			w:   want{kind: KindHTTP, host: "host", port: 5000}},
		...
		{name: "http path preserved",
			uri: "http://host:5000/api/v1",
			w:   want{kind: KindHTTP, host: "host", port: 5000, path: "/api/v1"}},
		{name: "grpc default port",
			uri: "grpc://host",
			w:   want{kind: KindGRPC, host: "host", port: 55055}},
		{name: "grpc explicit port",
			uri: "grpc://host:6000",
			w:   want{kind: KindGRPC, host: "host", port: 6000}},
		{name: "grpcs default port",
			uri: "grpcs://host",
			w:   want{kind: KindGRPCS, host: "host", port: 55555}},
...
func TestHTTPBaseURL(t *testing.T) {
	cases := []struct {
		...
		{"http://h:5000", "http://h:5000"},
		{"https://h:55555", "https://h:55555"},
		{"red://h:5050", ""},
	}
	for _, tc := range cases {
